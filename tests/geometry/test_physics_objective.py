...
def test_invalid_geometry_missing_edges_returns_inf():
    model = GeometryModel()
    model.points = {"A": (0,0,0), "B": (1,0,0)}
    model.edges = {}
    assert flare_law_acoustic_objective(model) == float('inf')

def test_single_point_returns_inf():
    model = GeometryModel()
...
def test_disconnected_components_returns_inf():
    model = GeometryModel()
...
def test_edges_not_covering_all_points_returns_inf():
    model = GeometryModel()
    model.points = {"A": (0,0,0), "B": (1,0,0), "C": (2,0,0)}
    model.edges = {"AB": ("A", "B")}
    assert flare_law_acoustic_objective(model) == float('inf')
